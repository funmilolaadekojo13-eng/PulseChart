/* PulseChart v2 — follow-up dashboard with WhatsApp reminders (template from Settings) */
...
function waLink(r) {
  ...
  const tmpl = CLINIC.reminder_template || DEFAULT_REMINDER;
  const text = encodeURIComponent(
    tmpl.replaceAll("{name}", honor + p.full_name)
        .replaceAll("{date}", when)
        .replaceAll("{practice}", CLINIC.practice_name || CONFIG.PRACTICE_NAME)
  );
